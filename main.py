...
async def rag_ingest_pdf(ctx: inngest.Context):
    def _load(ctx: inngest.Context) -> RAGChunkAndSrc:
        ...
        if not pdf_path:
            raise ValueError("Missing 'pdf_path' or 'pdf-path' in event data")
    ...
    ingested = await ctx.step.run("embe-and-upsert", lambda: _upsert(chunks_and_src), output_type=RAGUpsertResult)
...
async def rag_query_pdf_ai(ctx: inngest.Context):
    ...
    return {"answer": answer, "sources": list(found.sources), "num_contexts": len(found.contexts)}
